...
var max_page;
var page = 1;
function paging(which,max_page){
	var page_div = $(which).parent().parent().find('.paging');
	function pagehtml(){
		var PageStr = "",jj = "";
		page=parseInt(page);
	...
	function jump(which){
		...
		if(value != "" && r.test(value)){
			...
			page=value;
			$(page_div).html(pagehtml());
			var page_button=$(which).siblings('.page');
			$('.page').click(function(){
				pageClick(this);
			})
			$('.prev').click(function(){
	...
	function pageClick(which){
		...
		max_page=$(which).parent().attr('maxpage');
		page=value;
		$(page_div).html(pagehtml());
		$('.page').click(function(){
			pageClick(this);
